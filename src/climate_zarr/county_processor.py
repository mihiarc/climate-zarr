...
class ModernCountyProcessor:
    ...
    def process_zarr_data(
        ...
        # Debug: Check coordinate information
        console.print(f"[blue]Data dimensions: {list(data.dims)}[/blue]")
        console.print(f"[blue]Data coordinates: {list(data.coords)}[/blue]")
        
        # Handle antimeridian crossing for longitude coordinates
        from .utils.spatial_utils import normalize_longitude_coordinates
        if 'lon' in data.coords:
            lon_range = (float(data.lon.min()), float(data.lon.max()))
            gdf = normalize_longitude_coordinates(gdf, lon_range)
        elif 'longitude' in data.coords:
            lon_range = (float(data.longitude.min()), float(data.longitude.max()))
            gdf = normalize_longitude_coordinates(gdf, lon_range)
        
        # Ensure proper spatial coordinate setup for rioxarray
        try:
            # Try to set up CRS if not already present
            if not hasattr(data, 'rio') or data.rio.crs is None:
                console.print("[yellow]Setting up spatial reference for rioxarray...[/yellow]")
                # Assume WGS84 if no CRS is set
                data = data.rio.write_crs("EPSG:4326")
        except Exception as e:
            console.print(f"[yellow]Warning: Could not set up spatial reference: {e}[/yellow]")
            # Continue without CRS - some operations may still work
