...
climate-zarr/
├── src/climate_zarr/
│   ├── climate_cli.py          # 🎯 Interactive CLI tool
│   ├── stack_nc_to_zarr.py     # NetCDF → Zarr conversion
│   ├── county_processor.py     # 🏗️ Modern modular processor
│   ├── county_stats_cli.py     # Simplified CLI interface
...
│   │   ├── tasmax_processor.py         # Daily max temperature
│   │   ├── tasmin_processor.py         # Daily min temperature
│   │   └── processing_strategies.py    # Processing strategies
...
├── examples/
│   └── modular_processing_example.py  # Usage examples
├── tests/                      # 🧪 Comprehensive test suite
│   ├── test_processors.py         # Unit tests for processors
│   ├── test_strategies.py         # Processing strategy tests
│   ├── test_utils.py              # Utility function tests
│   ├── test_modular_integration.py    # Integration tests
│   ├── test_end_to_end_modular.py     # End-to-end tests
...
### 🏗️ **Modular Architecture** (2025 Refactoring)

The toolkit has been completely refactored from a monolithic 1,667-line file into a clean, modular architecture:

- **🧩 Modular Processors**: Each climate variable has its own specialized processor
- **⚡ Processing Strategies**: Vectorized and ultra-fast strategies for different use cases  
- **🛠️ Utility Modules**: Reusable spatial and data processing functions
- **📁 Standardized Output**: Organized file structure with configurable naming conventions
- **🧪 Comprehensive Testing**: Full test coverage with unit, integration, and compatibility tests
- **📖 Backward Compatibility**: Existing code continues to work unchanged

### 📁 **Smart Output Management**

All outputs are now automatically organized using configurable standards:
...
│   │   ├── conus/
│   │   │   └── historical/
│   │   │       ├── conus_historical_pr_stats.csv
│   │   │       └── conus_historical_pr_stats.metadata.json
│   │   └── alaska/
│   └── tas/                  # Temperature data
├── zarr/                     # Zarr datasets
...
**Naming Conventions**:
- `descriptive`: `region_scenario_variable_stats.csv` (default)
- `simple`: `variable_region.csv`
- `detailed`: `region_scenario_variable_stats_YYYYMMDD.csv`
- `iso`: `YYYY-MM-DD_region_variable_scenario.csv`
...
export CLIMATE_BASE_OUTPUT_DIR="./my_climate_results"
export CLIMATE_NAMING_CONVENTION="detailed"  # simple|descriptive|detailed|iso
export CLIMATE_CREATE_SUBDIRS="true"         # Organize into subdirectories
...
config.output.naming_convention = "iso"
config.output.organize_by_variable = True
...
```bash
# Performance settings
export CLIMATE_WORKERS=8
export CLIMATE_MEMORY_LIMIT="8GB"
export CLIMATE_COMPRESSION="zstd"
export CLIMATE_COMPRESSION_LEVEL=5

# Python configuration
config = get_config()
config.processing.n_workers = 8
config.processing.use_distributed = True
...
Different naming conventions produce different file structures:

```bash
# Descriptive (default): region_scenario_variable_stats.csv
conus_historical_pr_stats.csv
alaska_ssp370_tasmax_stats.csv
...
# Detailed: region_scenario_variable_stats_YYYYMMDD.csv
conus_historical_pr_stats_20250115.csv
alaska_ssp370_tasmax_stats_20250115.csv

# ISO: YYYY-MM-DD_region_variable_scenario.csv
2025-01-15_conus_pr_historical.csv
2025-01-15_alaska_tasmax_ssp370.csv
```
