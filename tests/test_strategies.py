...
class TestStrategyComparison:
    ...
    def test_strategies_produce_similar_results(self, sample_counties, sample_precipitation_data):
        """Test that UltraFast strategy produces consistent results."""
        ...
        results_1 = ultrafast.process(small_data, small_counties, 'pr', 'test', 25.4, 2)
        results_2 = ultrafast.process(small_data, small_counties, 'pr', 'test', 25.4, 2)
        
        # Should have same structure
        assert len(results_1) == len(results_2)
        assert list(results_1.columns) == list(results_2.columns)
    
    def test_strategy_performance_characteristics(self, sample_counties):
        """Test performance characteristics of UltraFast strategy."""
        ultrafast = UltraFastStrategy()
        ...
        assert len(small_counties) == 1
        assert len(large_counties) == 30
        
        # Strategy should be able to process the data types
